    ...
    await sheets.spreadsheets.values.update({
      ...
      requestBody: {
        values: [[
          ...
          all.idEvenement || "-",             // ID Événement RDV (J)
        ]],
      },
    });
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    ...
    if (interaction.isButton() && interaction.customId === "rdvFormNext") {
      ...
      const idEvenement = new TextInputBuilder().setCustomId("idEvenement").setLabel("ID Événement RDV").setStyle(TextInputStyle.Short).setRequired(false);

      modal2.addComponents(
        new ActionRowBuilder().addComponents(heure),
        new ActionRowBuilder().addComponents(date),
        new ActionRowBuilder().addComponents(rebooking),
        new ActionRowBuilder().addComponents(idEvenement)
      );
